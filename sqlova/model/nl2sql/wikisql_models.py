...
class Seq2SQL_v1(nn.Module):
    def __init__(self, iS, hS, lS, dr, n_cond_ops, n_agg_ops, n_conn_tiaojian, old=False):
        # dr是dropout参数
        ...
        self.n_conn_tiaojian = n_conn_tiaojian #新增一个预测


        self.scnp = SCNP(iS, hS, lS, dr) # 预测sel个数
        self.scp = SCP(iS, hS, lS, dr) # 预测每个sel是第几列
        self.sap = SAP(iS, hS, lS, dr, n_agg_ops, old=old) # 预测agg,聚合函数
        self.sop = SOP(iS, hS, lS, dr, n_conn_tiaojian) # 预测多个cond之间的操作 cond_conn_op
        # 以下四个预测cond的四个值
        self.wnp = WNP(iS, hS, lS, dr) # 预测Where-Number（几个cond）
        self.wcp = WCP(iS, hS, lS, dr) # 预测Where-Column（cond第一列）
        self.wop = WOP(iS, hS, lS, dr, n_cond_ops) # 预测Where-Operation（cond第二列）
        self.wvp = WVP_se(iS, hS, lS, dr, n_cond_ops, old=old) # start-end-search-discriminative model \ 预测Where-Value

    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs,
                g_scn = None, g_sc=None, g_sa=None,  g_sop=None,g_wn=None, g_wc=None, g_wo=None, g_wvi=None,
                show_p_sc=False, show_p_sa=False,
                show_p_wn=False, show_p_wc=False,
                show_p_wo=False, show_p_wv=False):
        col_len = np.array(l_hs)
        x_emb_var = wemb_n
        col_inp_var = wemb_hpu
        x_len = l_n
        col_name_len = l_hpu
        col_num = l_hs
        # scn 预测sel个数
        ...
        if g_scn:
            ...
            print('g_scn/wn/wc/wo dev/test不监督')
        ...
        return s_scn, s_sc, s_sa, s_sop, s_wn, s_wc, s_wo, s_wv
    ...
    def beam_forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, engine, tb,
        ...
        """
        # sc select select sel
        s_sc = self.scp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc=show_p_sc)
        prob_sc = F.softmax(s_sc, dim=-1)
        bS, mcL = s_sc.shape #bS 行数  mcl 列数
        ...
        prob_sc_sa = torch.zeros([bS, beam_size, self.n_agg_ops]).to(device) #造一个以sc的行数 ,beam和agg个数的概率举证
        prob_sca = torch.zeros_like(prob_sc_sa).to(device)

        # get the top-k indices.  pr_sc_beam = [B, beam_size] #获得前k个概率
        ...
        prob_wn = F.softmax(s_wn, dim=-1).detach().to(device).numpy()
        ...
        # get max_wn # of most probable columns & their prob. 得到有可能的列和概率
    ...
    '''
    自己假的生成query函数。modle.query
    '''
    def gen_query(self, score,nlu_tt, nlu_t):
        """
        :param score:
        ...
        sel_num_score, sel_score, agg_score, where_rela_score = score
        # [64,4,6], [64,14], ..., [64,4]
...
class SCNP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3):
        ...
        self.sel_num_lstm = nn.LSTM(input_size=iS, hidden_size=int(hS / 2),
        ...
        self.sel_num_out = nn.Sequential(nn.Linear(hS, hS),
                                         nn.Tanh(), nn.Linear(hS, 4)) # 输出是4(sqlnet)还是1?
        self.softmax = nn.Softmax(dim=-1) # dim=-1,让张量按照内部行归一化概率
...
class WNP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wn=False):
        ...
        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs)  # [b, mL_hs, dim] header（mL_hs）的嵌入表示，每个header都是dim维向量表示
...
class SOP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3,n_conn_tiaojian=-1):
        ...
        self.where_rela_lstm = nn.LSTM(input_size=iS, hidden_size=int(hS // 2),
                             num_layers=lS, batch_first=True,
                             dropout=dr, bidirectional=True)
        self.where_rela_att = nn.Linear(hS, 1)
        self.where_rela_col_att = nn.Linear(hS, 1)
        self.where_rela_out = nn.Sequential(
            nn.Linear(hS, hS), nn.Tanh(), nn.Linear(hS, n_conn_tiaojian)
        )
        self.softmax = nn.Softmax(dim=-1)
        self.col2hid1 = nn.Linear(hS, 2 * hS)
        self.col2hid2 = nn.Linear(hS, 2 * hS)

    def forward(self,x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_nums):
        ...
        K_num_col = (e_num_col * num_col_att.unsqueeze(2)).sum(1)
        h1 = self.col2hid1(K_num_col).view(B, 4, self.hS // 2).transpose(0, 1).contiguous()
        h2 = self.col2hid2(K_num_col).view(B, 4, self.hS // 2).transpose(0, 1).contiguous()
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wo, wenc_n=None, show_p_wv=False):
        ...
        if not wenc_n:
            wenc_n, hout, cout = encode(self.enc_n, wemb_n, l_n,
                            return_hidden=True,
                            hc0=None,
                            last_only=False)  # [b, n, dim] 问题（n）的嵌入表示，每个问题都是dim维向量表示

        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs)  # [b, hs, dim] header（mL_hs）的嵌入表示，每个header都是dim维向量表示
        ...
        # Column attention
        # [B, 1, mL_n, dim] * [B, 4, dim, 1]
        #  -> [B, 4, mL_n, 1] -> [B, 4, mL_n] 矩阵乘法广播机制
        # multiplication bewteen NLq-tokens and  selected column
        att = torch.matmul(self.W_att(wenc_n).unsqueeze(1),
                           wenc_hs_ob.unsqueeze(3)
                           ).squeeze(3)
        # 问题+headers嵌入矩阵乘法得到att
        # Penalty for blank part.
        mL_n = max(l_n)
        for b, l_n1 in enumerate(l_n):
            if l_n1 < mL_n:
                att[b, :, l_n1:] = -10000000000
                # 计算att的时候是按照最大的问题长度长度填充的，这代码为了消除小于max的充填部分概率
...
def Loss_sw_se(s_scn, s_sc, s_sa, s_sop, s_wn, s_wc, s_wo, s_wv,
    ...
    """
    loss = 0
    loss += Loss_scn(s_scn, g_sel_num_seq)
    loss += Loss_sc(s_sc, g_sc)
    loss += Loss_sa(s_sa, g_sa)
    loss += Loss_sop(s_sop, g_sop)
...
def Loss_scn(s_scn, g_sel_num_seq):
    loss = F.cross_entropy(s_scn, torch.tensor(g_sel_num_seq).to(device))
    return loss
...
def Loss_sop(s_sop, g_sop):
    data = torch.from_numpy(np.array(g_sop))
    try:
        where_rela_truth = Variable(data.cuda())
    except:
        print("where_rela_truth error")
        exit(0)
    loss = F.cross_entropy(s_sop, where_rela_truth)
