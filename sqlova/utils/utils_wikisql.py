...
def get_fields_1(t1, tables, no_hs_t=False, no_sql_t=False):
    ...
    return nlu1, nlu_t1, tid1, sql_i1, sql_t1, tb1, hs_t1, hs1

def get_fields(t1s, tables, no_hs_t=False, no_sql_t=False):

    nlu, nlu_t, tid, sql_i, sql_q, sql_t, tb, hs_t, hs, col_seq = [], [], [], [], [], [], [], [], [], []
    for t1 in t1s:
        if no_hs_t:
            nlu1, nlu_t1, tid1, sql_i1, sql_t1, tb1, hs_t1, hs1 = get_fields_1(t1, tables, no_hs_t, no_sql_t)
        else:
            nlu1, nlu_t1, tid1, sql_i1, sql_t1, tb1, hs_t1, hs1 = get_fields_1(t1, tables, no_hs_t, no_sql_t)
        ...
        hs_t.append(hs_t1)
        hs.append(hs1)
        # col_seq.append(col_seq1)

    return nlu, nlu_t, sql_i, sql_t, tb, hs_t, hs
...
def encode(lstm, wemb_l, l, return_hidden=False, hc0=None, last_only=False):
    ...
    l = array(l) # narray数组
    perm_idx = argsort(-l) # 从小到大排序 倒置 返回索引大小
    perm_idx_inv = generate_perm_inv(perm_idx) # 从小到大，索引的索引值
    ...
    packed_wemb_l = nn.utils.rnn.pack_padded_sequence(wemb_l[perm_idx, :, :],
                                                      l[perm_idx],
                                                      batch_first=True)
    #pack_padded_sequence函数的参数，第二个参数lengths需要从大到小排序，
    # x(第一个参数）为已根据长度大小排好序，batch_first如果设置为true，则x的第一维为batch_size，第二维为seq_length，否则相反
...
def encode_hpu(lstm, wemb_hpu, l_hpu, l_hs):
    ...
    return wenc_hs # 返回的是每个表头的lstm矩阵,并且小于最大header个数的部分都是0. 每个header都是100维向量表示
...
def get_g(sql_i):
    ...
    cond = []
    '''
    这里可能需要改，agg和sel的加入方式，是二维数组还是一位数组，欠考虑
    '''

    for b, psql_i1 in enumerate(sql_i):
        sel_num = len(psql_i1['sel'])
        g_sel_num_seq.append(sel_num)
        g_sel_ag_seq.append(
                (len(psql_i1['agg']),
                 psql_i1['sel'],
                 psql_i1['agg'],
        ...
        g_sop.append(psql_i1['cond_conn_op'])
        conds = psql_i1['conds']
        cond.append(psql_i1['conds'])
    ...
    return g_sc, g_sa, g_sop, g_wn, g_wc, g_wo, g_wv, g_sel_num_seq, g_sel_ag_seq,cond
...
def get_bert_output(model_bert, tokenizer, nlu_t, hds, max_seq_length):
    ...
    for b, nlu_t1 in enumerate(nlu_t):
        ...
        nlu_tt.append(nlu_tt1)
        tt_to_t_idx.append(tt_to_t_idx1)
        t_to_tt_idx.append(t_to_tt_idx1) #每个word spice长度
...
def pred_sw_se(s_sop, s_wn, s_wc, s_wo, s_wv):
    pr_sop = pred_sa(s_sop)
    pr_wn = pred_wn(s_wn)
    pr_wc = pred_wc(pr_wn, s_wc)
    pr_wo = pred_wo(pr_wn, s_wo)
    pr_wvi = pred_wvi_se(pr_wn, s_wv)

    return  pr_wn, pr_wc, pr_sop, pr_wo, pr_wvi





def merge_wv_t1_eng(tok_list, raw_tok_str):
    """
...
def get_cnt_sw_list(g_sc, g_sa, g_wn, g_wc, g_sop, g_wo, g_wvi,
                    pr_sc, pr_sa, pr_wn, pr_wc, pr_sop, pr_wo, pr_wvi,
                    g_sql_i, pr_sql_i,
                    mode):
    """ usalbe only when g_wc was used to find pr_wv
    """
    ...
    cnt_sop = get_cnt_sc_list(g_sop, pr_sop)
    ...
    return cnt_sc, cnt_sa, cnt_wn, cnt_wc, cnt_sop, cnt_wo, cnt_wvi, cnt_wv


def get_cnt_lx_list(cnt_sc1, cnt_sa1, cnt_wn1, cnt_wc1, cnt_sop1, cnt_wo1, cnt_wv1):
    # all cnt are list here.
    cnt_list = []
    cnt_lx = 0
    for csc, csa, cwn, cwc, csop, cwo, cwv in zip(cnt_sc1, cnt_sa1, cnt_wn1, cnt_wc1, cnt_sop1, cnt_wo1, cnt_wv1):
        if csc and csa and cwn and cwc and csop and cwo and cwv:
            cnt_list.append(1)
        else:
            cnt_list.append(0)
...
def generate_sql_i(pr_sql_i1, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu):
    pr_sql_i = []
    for b, nlu1 in enumerate(nlu):
        ...
        for i_wn in range(pr_wn[b]):
            ...
            conds.append(conds1)
            pr_sql_i1[b].update({'conds': conds})
...
def get_g_wvi_corenlp_gt(gt_where_seq):
    ...
    for b in range(len(gt_where_seq)):
        gt1 = []
        for x in range(len(gt_where_seq[b])):
            if len(gt_where_seq[b][x])==2:
                gt1.append([gt_where_seq[b][x][0],gt_where_seq[b][x][1]])
            else:
                gt1.append([gt_where_seq[b][x][1],gt_where_seq[b][x][len(gt_where_seq[b][x])-2]])
        gt.append(gt1)
